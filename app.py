...
# ==========================================
# 2. NGÔN NGỮ THIẾT KẾ PHÔNG NỀN XANH THẪM - CHỮ VÀNG GOLD LUXURY STYLE
# ==========================================
st.markdown("""
    <style>
    /* Nền thẫm tối thượng chuẩn J.P. Morgan & Morgan Stanley */
    ...
    /* Làm nổi bật tiêu đề bằng màu vàng cát sang trọng */
    ...
    .founder-name { font-size: 24px; font-weight: 800; color: #D4AF37 !important; margin-top: 15px; margin-bottom: 2px; }
    .founder-title { font-size: 13px; color: #E2E8F0 !important; text-transform: uppercase; letter-spacing: 2px; font-weight: 700; }
    ...
    .price-card-title { font-size: 18px; font-weight: 800; color: #D4AF37 !important; text-transform: uppercase; letter-spacing: 1px; }
    .price-card-amount { font-size: 34px; font-weight: 900; color: #FFFFFF !important; margin: 15px 0px 5px 0px; }
    
    /* Gói Thượng tầng VIP đổi nền vàng chữ đen hoặc giữ đen viền vàng */
    .price-grid-box.vip-tier {
        background-color: #1E293B;
        border: 2px solid #F5A623;
    }
    .price-grid-box.vip-tier .price-card-title { color: #F5A623 !important; }
    
    /* Định hình thanh đóng mở Expanders nền thẫm viền vàng cát */
    ...
    /* Ô điền dữ liệu input sáng sủa dễ nhìn */
    input, select {
        background-color: #112240 !important;
        color: #FFFFFF !important;
        border: 1px solid #D4AF37 !important;
    ...
    /* Nút bấm vàng gold chữ đen cực kỳ sang trọng */
    button {
    ...
    /* Định hình lại các dòng text trong màng lọc định giá trích xuất */
    .highlight-text-box {
...
st.markdown("""
    <div class="premium-header">
        <div class="premium-title">Pentech Premium <span style='font-size:16px; color:#FFFFFF; font-weight:600;'>INSTITUTIONAL TERMINAL</span></div>
        <div class="premium-subtitle">Hạ tầng Real-time 3 sàn • Giao diện Midnight Royal Blue & Luxury Gold</div>
    </div>
""", unsafe_allow_html=True)
...
# Tạo khung mô tả biệt lập 6 dòng thực chiến rạch ròi cho từng bài học
...
corporate_market_db = {
    ...
    "MSN": {"name": "CTCP Tập Đoàn Masan", "exchange": "HOSE", "sector": "HÀNG TIÊU DÙNG THIẾT YẾU", "eps": 2950, "growth": 14, "roe": 11.2, "roi": 9.5, "moat": "Hệ sinh thái bán lẻ tiêu dùng khép kín lớn nhất nội địa", "fallback_price": 76500},
...
def get_live_stock_price(ticker):
    ...
    if clean_tk in corporate_market_db:
        ...
        return {"name": f"Doanh nghiệp niêm yết ({clean_tk})", "exchange": "HOSE", "sector": "SẢN XUẤT", "eps": 3200, "current": final_price, "growth": 12, "roe": 14.5, "roi": 11.2, "moat": "Hệ số cạnh tranh"}

# ==========================================
# 5. KHỐI HIỂN THỊ CHÂN DUNG CEO ĐỊNH CHẾ LUXURY STYLE
# ==========================================
with st.expander("💎 SỨ MỆNH PHỤNG SỰ & KHỞI TRẠM CÔNG NGHỆ TƯƠNG LAI CAO CẤP", expanded=True):
    ...
    with col_founder_img:
        ...
        if os.path.exists(fixed_img_path):
            ...
            st.markdown("""<div class="founder-card"><img src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=200&auto=format&fit=crop" class="founder-avatar"><div class="founder-name">Trần Anh Quân</div><div class="founder-title">Nhà sáng lập & CEO</div></div>""", unsafe_allow_html=True)
            
    with col_mission_text:
        st.markdown(f"""
            <h3 style='color:#D4AF37 !important; margin-top:0; font-weight:800;'>Hạ tầng tri thức định lượng dẫn dắt bởi nhà sáng lập Trần Anh Quân</h3>
            <p style='font-size:16px; line-height:1.7; color:#E2E8F0; text-align: justify;'>
...
with st.expander("⚙️ BAN ĐIỀU HÀNH: Tải tệp ảnh khóa cứng vĩnh cửu"):
    uploaded_image = st.file_uploader("Tải ảnh chân dung mới của bạn để khóa cứng vào mã nguồn hệ thống:", type=["jpg", "jpeg", "png"])
    if uploaded_image is not None:
        with open("founder_fixed.jpg", "wb") as f: f.write(uploaded_image.getbuffer())
        st.success("🎉 Đã lưu ảnh thành công vào core hệ thống vĩnh cửu!")

# ==========================================
# 6. TRẠM TRA CỨU TƯƠNG TÁC THÔNG MINH RỔ VN30 KỲ 1/2026
# ==========================================
...
# LỆNH LẬP TRÌNH TƯƠNG TÁC: Khi click chọn mã cổ phiếu sẽ hiển thị ngay sàn giao dịch, phân ngành vĩ mô thuộc rổ VN30
selected_ticker = st.selectbox("👉 HỌC VIỆN: Click chọn mã cổ phiếu bất kỳ trong rổ VN30 để bóc tách thông tin tự động:", vn30_official)
ticker_data = get_live_stock_price(selected_ticker)

st.markdown(f"""
    <div class="compare-box" style="border-left: 8px solid #D4AF37; background-color: #112240;">
        <h4 style='margin-top:0; border-bottom:2px solid #D4AF37; padding-bottom:5px; font-weight:800; color:#D4AF37;'>🔍 ĐỐI CHIẾU THÔNG TIN ĐỊNH HƯỚNG TÀI SẢN: {selected_ticker}</h4>
        <p style='font-size:16px;'>• Tên Doanh Nghiệp Đầy Đủ: <b style="color:#FFFFFF;">{ticker_data['name']}</b></p>
        <p style='font-size:16px;'>• Thị Trường Niêm Yết: <span style="background-color:#D4AF37; color:#0A192F; padding:2px 8px; font-weight:800; font-size:13px; border-radius:2px;">{ticker_data['exchange']}</span> | Vị thế: <b>Thuộc rổ chỉ số rường cột VN30 Kỳ 1/2026 Việt Nam</b></p>
        <p style='font-size:16px;'>• Phân Nhánh Kỹ Nghệ / Ngành: <b style="background-color:#0A192F; padding:3px 8px; border:1px solid #D4AF37; color:#D4AF37 !important;">{ticker_data['sector']}</b></p>
        <p style='font-size:16px;'>• Giá Real-time khớp lệnh chuẩn xác: <b style="font-size:26px; color:#FFFFFF;">{ticker_data['current']:,.0f} VNĐ</b></p>
        <div class="highlight-text-box">
            <p style="color:#D4AF37 !important; margin:0; font-size:15px; font-weight:700;">💎 ĐỊNH GIÁ TRÍCH XUẤT TỪ TERMINAL: EPS {ticker_data['eps']:,.0f} VNĐ | ROE {ticker_data['roe']:.1f}% | ROI {ticker_data['roi']:.1f}% | Tăng trưởng chu kỳ: +{ticker_data['growth']}%</p>
        </div>
        <p style='font-size:15px;'>• Cơ Chế Biện Giải Hào Bảo Vệ Kinh Tế (Moat): <i style="color:#E2E8F0;">{ticker_data['moat']}</i></p>
    </div>
""", unsafe_allow_html=True)

with st.expander("📊 Xem bảng tổng hợp toàn rổ chỉ số VN30 (Dữ liệu lưới Real-time)"):
    grid_rows = []
...
with st.expander("⏳ Xem bảng danh mục cổ phiếu dự phòng chỉ số VN30 Kỳ 1/2026"):
    grid_rows_back = []
...
st.markdown("<br>### 🎛️ TERMINAL ĐỐI CHIẾU SONG SUNG ĐA TÀI SẢN TÙY CHỌN LIÊN SÀN", unsafe_allow_html=True)
...
with col_box1:
    st.markdown(f"""<div class="compare-box"><h4 style='margin-top:0; border-bottom:1px solid #D4AF37; padding-bottom:5px; color:#D4AF37; font-weight:800;'>📊 TRẠM A: {tkA} (Sàn: {data_A['exchange']})</h4><p>• Doanh nghiệp: <b>{data_A['name']}</b></p><p>• Giá chuẩn xác: <b style='font-size:20px; color:#FFFFFF;'>{data_A['current']:,.0f} VNĐ</b></p><p>• <b>ĐỊNH GIÁ: EPS {data_A['eps']:,.0f} VNĐ | ROE {data_A['roe']:.1f}% | ROI {data_A['roi']:.1f}%</b></p></div>""", unsafe_allow_html=True)
with col_box2:
    st.markdown(f"""<div class="compare-box"><h4 style='margin-top:0; border-bottom:1px solid #D4AF37; padding-bottom:5px; color:#D4AF37; font-weight:800;'>📊 TRẠM B: {tkB} (Sàn: {data_B['exchange']})</h4><p>• Doanh nghiệp: <b>{data_B['name']}</b></p><p>• Giá chuẩn xác: <b style='font-size:20px; color:#FFFFFF;'>{data_B['current']:,.0f} VNĐ</b></p><p>• <b>ĐỊNH GIÁ: EPS {data_B['eps']:,.0f} VNĐ | ROE {data_B['roe']:.1f}% | ROI {data_B['roi']:.1f}%</b></p></div>""", unsafe_allow_html=True)

# Biểu đồ dòng tiền
dates = [datetime.now() - timedelta(days=x) for x in range(100, 0, -1)]
fig = go.Figure()
fig.add_trace(go.Scatter(x=dates, y=[data_A['current'] * (0.95 + (i*0.001)) for i in range(100)], mode='lines', name=tkA, line=dict(color='#D4AF37', width=3)))
fig.add_trace(go.Scatter(x=dates, y=[data_B['current'] * (0.94 + (i*0.0012)) for i in range(100)], mode='lines', name=tkB, line=dict(color='#FFFFFF', width=1.5, dash='dot')))
fig.update_layout(paper_bgcolor="#0A192F", plot_bgcolor="#112240", margin=dict(l=10, r=10, t=10, b=10), height=240, legend=dict(font=dict(color="#FFFFFF", size=12)), xaxis=dict(gridcolor="#1E293B", tickfont=dict(color="#FFFFFF", size=12)), yaxis=dict(gridcolor="#1E293B", tickfont=dict(color="#FFFFFF", size=12)))
...
with col_f1:
    st.markdown("""<div class="strategy-card"><div class="book-tag" style="background-color:#D4AF37 !important; color:#0A192F !important;">KỶ NGUYÊN SỐ</div><h4 style='margin-top:0; font-weight:800; color:#D4AF37 !important;'>1. CÔNG NGHỆ BÁN DẪN & AI ĐỊNH LƯỢNG</h4><p style='font-size:14px;'>Hạ tầng vi mạch và các thuật toán máy học tự động hóa (Tiêu biểu như FPT) nắm giữ độc quyền phân phối và tăng trưởng bền vững dài hạn.</p></div>""", unsafe_allow_html=True)
with col_f2:
    st.markdown("""<div class="strategy-card"><div class="book-tag" style="background-color:#D4AF37 !important; color:#0A192F !important;">HẠ TẦNG KẾT NỐI</div><h4 style='margin-top:0; font-weight:800; color:#D4AF37 !important;'>2. VIỄN THÔNG 5G & LOGISTICS SỐ</h4><p style='font-size:14px;'>Mạng lưới trạm phát sóng liên quốc gia và chuỗi vận tải chuyển phát nhanh khép kín (Tiêu biểu như VGI, CTR, VTP) phòng vệ lạm phát tối ưu.</p></div>""", unsafe_allow_html=True)
with col_f3:
    st.markdown("""<div class="strategy-card"><div class="book-tag" style="background-color:#D4AF37 !important; color:#0A192F !important;">TIÊU DÙNG THIẾT YẾU</div><h4 style='margin-top:0; font-weight:800; color:#D4AF37 !important;'>3. TIÊU DÙNG SẠCH & Y TẾ CHUỖI ĐỘC QUYỀN</h4><p style='font-size:14px;'>Sự bùng nổ nhu cầu thực phẩm đóng gói thương hiệu và chuỗi dược phẩm bán lẻ (Tiêu biểu như MCH, FRT) bền vững bất chấp chu kỳ suy thoái.</p></div>""", unsafe_allow_html=True)
    ...
    if strat["id"] <= 15:
        ...
        if is_unlocked:
            with st.expander(f"🔓 BÀI HỌC {strat['id']}: {strat['title'].upper()} (ĐÃ KÍCH HOẠT VIP)"):
                st.markdown(f"""<div class="strategy-card" style="border-color: #D4AF37;"><p style='font-size:15px; line-height:1.7; white-space: pre-wrap;'>{strat['desc']}</p></div>""", unsafe_allow_html=True)
        else:
            with st.expander(f"🔒 BÀI HỌC {strat['id']}: [BỊ KHÓA] NÂNG CẤP GÓI ĐỂ MỞ KHÓA"):
                st.markdown("""<div class="locked-card"><h4>🔒 Nội dung bài học thuộc quyền sở hữu của Gói 2 & Gói 3</h4><p style='color:#F5A623 !important;'>Bạn đang sử dụng tài khoản Gói Cơ Bản. Để mở khóa quy tắc quản trị rủi ro tối cao nâng cao nâng cấp... vui lòng nhập mã kích hoạt (License Key) từ CEO Trần Anh Quân để bẻ khóa hệ thống.</p></div>""", unsafe_allow_html=True)
...
with col_form:
    with st.form("institutional_contact", clear_on_submit=True):
        st.markdown("<b style='color:#D4AF37; font-size:16px;'>📩 ĐĂNG KÝ THAM GIA KHÓA HỌC & ỦY THÁC HỢP TÁC CHIẾN LƯỢC VIP</b>", unsafe_allow_html=True)
...
with col_contact:
    st.markdown(f"""<div style="background-color: #112240; padding: 25px; border: 1px solid #D4AF37; height: 195px; border-radius: 4px;"><span style="color: #D4AF37; font-size: 12px; display: block; margin-bottom: 5px; font-weight:700; letter-spacing:1px;">🏢 ĐƯỜNG DÂY NÓNG BAN ĐIỀU HÀNH QUỸ</span><span style="font-size: 30px; font-weight: 900; color: #FFFFFF; display: block; letter-spacing: -1px;">0327.625.853</span><p style="font-size: 14px; color: #E2E8F0; margin-top: 12px; line-height: 1.5; font-weight: 500;">Liên hệ trực tiếp với Ban điều hành Pentech Premium qua Hotline/Zalo cá nhân của Mr. Trần Anh Quân: <b style='color:#D4AF37;'>0327.625.853</b> để nhận giải pháp cơ cấu tài sản và cấu hình bảo mật thông tin.</p></div>""", unsafe_allow_html=True)
...
st.markdown("""
    <div style="border-top: 1px solid #D4AF37; padding-top: 20px; color: #E2E8F0; font-size: 12px; line-height: 1.6; font-weight: 500;">
        <b style="color: #D4AF37; font-size: 14px; display: block; margin-bottom: 5px;">💎 PENTECH PREMIUM FINANCIAL TECHNOLOGY CORPORATION</b>
        ...
        <br><br>
        <div style="text-align: center; color: #D4AF37; font-weight: bold;">© 2026 Pentech Premium. All corporate rights reserved. Power of Quantitative Python Logics.</div>
    </div>
""", unsafe_allow_html=True)
