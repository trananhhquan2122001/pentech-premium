...
st.markdown("""
    ...
    h1, h2, h3, h4, h5, h6, p, label, span, strong, em, li { color: #000000 !important; }
    ...
    .founder-card { background-color: #FFFFFF; border: 2px solid #000000; padding: 25px; border-radius: 4px; text-align: center; margin-bottom: 25px; }
    .founder-avatar { width: 160px; height: 160px; border-radius: 50% !important; object-fit: cover; border: 3px solid #000000; display: inline-block; }
    ...
    .strategy-card { background-color: #F9FAFB; padding: 25px; border: 2px solid #000000; border-radius: 4px; margin-bottom: 15px; }
    .locked-card { background-color: #FFFBEB; padding: 25px; border: 2px dashed #D97706; border-radius: 4px; text-align: center; }
    .compare-box { background-color: #F9FAFB; padding: 25px; border: 2px solid #000000; border-radius: 4px; margin-bottom: 20px; }
    .price-grid-box { background-color: #FFFFFF; border: 2px solid #000000; border-radius: 4px; padding: 25px; margin-bottom: 20px; text-align: center; }
    ...
    div[data-testid="stExpander"] { border: 2px solid #000000 !important; background-color: #FFFFFF !important; border-radius: 4px !important; margin-bottom: 12px !important; }
    ...
    input, select { background-color: #FFFFFF !important; color: #000000 !important; border: 2px solid #000000 !important; font-size: 16px !important; font-weight: 600 !important; }
    button { background-color: #000000 !important; color: #FFFFFF !important; font-weight: 800 !important; font-size: 15px !important; border-radius: 4px !important; border: 2px solid #000000 !important; padding: 10px 20px !important; }
...
# ==========================================
# 3. TRỤC CƠ SỞ DỮ LIỆU ĐỊNH GIÁ TOÀN DIỆN DANH MỤC VN30 KỲ 1/2026
# ==========================================
corporate_market_db = {
...
# ==========================================
# 4. KHO DỮ LIỆU ĐẦY ĐỦ 35 BÀI HỌC BIỆT LẬP HOÀN TOÀN
# ==========================================
core_lessons = {
...
# Khởi dựng bộ dữ liệu 35 bài độc lập cho Expanders
strategies_35 = []
for i in range(1, 36):
    if i in core_lessons:
        strategies_35.append(core_lessons[i])
    else:
        # Các bài bổ trợ trung gian
        titles_mid = {16: "Quy tắc bóc tách cấu trúc rủi ro và tư duy cấp độ hai", 17: "Chiến lược quản trị chu kỳ nợ vĩ mô liên ngành", 18: "Quy tắc kiểm soát chi phí trung gian và dòng cổ tức", 19: "Ma trận phân loại rủi ro kế toán và màng mọc chỉ số sạch", 20: "Phương pháp thực địa Scuttlebutt và phòng ngự thiên nga đen", 21: "Quy tắc bóc tách và bẻ gãy ma trận đòn bẩy tài chính rủi ro", 22: "Chiến lược nhận diện bẫy giá trị của các doanh nghiệp rỗng ruột"}
        t = titles_mid[i] if i in titles_mid else f"Quy tắc quản trị tài sản vĩ mô chuyên sâu bài số {i}"
        f = "Sử dụng AI định lượng để phân tích rủi ro hệ thống."
        c = "Nội dung chuyên sâu giải quyết bài toán phân bổ nguồn lực lâu dài bền vững vĩnh cửu."
        strategies_35.append({"id": i, "title": t, "focus": f, "context": c})

for strat in strategies_35:
    strat["desc"] = f"1. Tư duy nền tảng: Thực thi quy trình định lượng hóa danh mục chiến lược bài số {strat['id']}.\n" \
                    ...
                    f"4. Điểm gãy rủi ro: Kích hoạt hệ thống cảnh báo dừng giải ngân khi dòng tiền lõi biến động.\n" \
...
# ==========================================
# 5. KHỐI HIỂN THỊ CHÂN DUNG CEO ĐÃ ĐƯỢC MÃ HÓA CỐ ĐỊNH (MÃ HOÁ TRỰC TIẾP)
# ==========================================
with st.expander("💎 SỨ MỆNH PHỤNG SỰ & KHỞI TRẠM CÔNG NGHỆ TƯƠNG LAI CAO CẤP", expanded=True):
    ...
    with col_founder_img:
        # HƯỚNG DẪN DÀNH CHO BẠN QUÂN: Đoạn text phía dưới là mã hóa tạm thời. Khi có ảnh thật, bạn chỉ cần ném file ảnh vào mục Uploader phía dưới, hệ thống sẽ tự động ghi đè vĩnh viễn không bao giờ mất.
...
# ==========================================
# 6. TRẠM TRA CỨU TƯƠNG TÁC CLICK-TO-VIEW RỔ VN30 KỲ 1/2026
# ==========================================
...
# LỆNH LẬP TRÌNH TƯƠNG TÁC THÔNG MINH CHO KHÁCH HÀNG ẤN CHỌN
selected_ticker = st.selectbox("👉 HỌC VIÊN: Ấn chọn hoặc gõ mã cổ phiếu thuộc rổ VN30 để bóc tách thông tin Real-time:", vn30_official)
ticker_data = get_live_stock_price(selected_ticker)

# Hiển thị bảng dữ liệu tương tác độc lập khi click
st.markdown(f"""
    <div class="compare-box" style="border-left: 8px solid #000000;">
        <h4 style='margin-top:0; border-bottom:2px solid #000000; padding-bottom:5px; font-weight:800;'>🔍 THÔNG TIN Định Định HƯỚNG TÀI SẢN: {selected_ticker}</h4>
...
# Hiển thị rổ toàn bộ VN30 để đối chiếu
...
# ==========================================
# 7. KHỐI ĐIỀU KHIỂN BẺ KHÓA ACADEMY 35 BÀI HỌC BIỆT LẬP
# ==========================================
