...
st.markdown("""
    ...
    /* ĐỊNH DẠNG CSS ÉP ẢNH BO TRÒN TUYỆT ĐỐI VÀ ĐỔ BÓNG KHỐI CHUYÊN NGHIỆP */
    .founder-frame {
        text-align: center;
        padding: 10px;
    }
...
# ==========================================
# 🌟 KHỐI HIỂN THỊ CHÚNG TÔI LÀ AI & ENGINE TỰ GHI ĐÈ ẢNH VĨNH VIỄN
# ==========================================
with st.expander("💎 CHÚNG TÔI LÀ AI & SỨ MỆNH ĐỊNH CHẾ PHỤNG SỰ"):
    
    # Khu vực nút bấm cho phép bạn Quân tải ảnh lên trực tiếp từ máy để hệ thống tự lưu vĩnh viễn
    uploaded_image = st.file_uploader("⚙️ BAN ĐIỀU HÀNH: Tải ảnh chân dung từ thiết bị lên để cài đặt hiển thị mặc định vĩnh viễn:", type=["jpg", "jpeg", "png"])
    
    # Thuật toán ngầm: Nếu phát hiện có ảnh mới được tải lên, tự động ghi đè lưu thành file 'founder_fixed.jpg' vĩnh viễn trên máy chủ
    fixed_img_path = "founder_fixed.jpg"
    if uploaded_image is not None:
        with open(fixed_img_path, "wb") as f:
            f.write(uploaded_image.getbuffer())
        st.success("🎉 Hệ thống đã đồng bộ và cấu hình bức ảnh này làm ảnh mặc định vĩnh viễn thành công! Vui lòng F5 trang web để kiểm tra.")
    
    st.markdown("<br>", unsafe_allow_html=True)
    col_img, col_text = st.columns([4, 7])
    
    with col_img:
        # Kiểm tra xem file lưu vĩnh viễn đã tồn tại chưa
        if os.path.exists(fixed_img_path):
            ...
            # Avatar bảo hộ đồ họa tạm thời khi bạn chưa thực hiện bấm nút tải ảnh lần nào
            st.markdown("""
                <div class="founder-frame">
...
corporate_database = {
    ...
    "VCB": {"name": "Vietcombank", "eps": 6800, "current": 94000, "growth": 18, "roe": 21.0, "roi": 15.2, "moat": "Thương hiệu ngân hàng quốc doanh vị thế bán lẻ tuyệt đối", "reflexivity": "Trục xương sống dòng vốn quốc gia"}
}
...
HNX_200 = [
    ...
    "KOS", "DST", "SRA", "AMC", "DTD", "NSH", "PJC", "TTD", "VNT", "TMC", "MAS", "SGC", "DNC", "SED", "EBS", "SGD", "DAD", "STC", "DAE", "ALT",
    "UNI", "CIA", "BLF", "SGH", "VTS", "NAG", "PGI", "PTI", "VNR", "PRE", "TAS", "HFT", "TVA", "S99", "SCI", "MTA", "VHE", "VNA", "VST", "VSG",
    "VGG", "MML", "BRR", "MSR", "VEF", "BHT", "CDO", "CLG", "FTM", "KSA", "PTM", "SGO", "VNH", "VTE", "KMT", "HFX", "KTT", "L44", "L61", "L62",
    "MAX", "PVR", "S91", "S74", "SD4", "SD5", "SD6", "SD9", "SDA", "SDB", "SDC", "SDE", "SDG", "SDH", "SDP", "SDT", "SDU", "SDY", "SIC", "SJE", "SNG"
]
...
with col_input1:
    tk1_raw = st.text_input("MÃ DOANH NGHIỆP A:", value="VPB")
    data_A, fair_A, margin_A, upside_A = get_stock_data(tk1_raw)
    tk1 = tk1_raw.strip().upper()
with col_input2:
    tk2_raw = st.text_input("MÃ DOANH NGHIỆP B:", value="TCB")
